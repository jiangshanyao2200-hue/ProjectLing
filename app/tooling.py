...
def _aidebug_root(config: Any) -> Path:
    configured = os.environ.get("AITERMUX_AIDEBUG_DIR")
    if configured:
        return Path(configured).expanduser()
    root_dir = _root_dir(config)
    direct = root_dir / "aidebug"
    if direct.exists() or (root_dir / "run.sh").exists() or (root_dir / "projectling.py").exists():
        return direct
    return root_dir / "projectling" / "aidebug"
...
def _execute_aidebug_read(args: dict[str, Any], context: ToolContext) -> dict[str, Any]:
    ...
    try:
        file_lines = path.read_text(encoding="utf-8", errors="replace").splitlines(keepends=True)
    ...
    if mode == "head":
        ...
        text = "".join(file_lines[start_line - 1 : end_line])
    else:
        mode = "tail"
        text = "".join(file_lines[-lines:])
...
def _resolve_patch_target(cwd: Path, path: str) -> Path:
    token = _normalize_patch_path_token(path)
...
def _patch_target_safety_reason(target: Path) -> str:
    resolved = target.resolve(strict=False)
    lowered_parts = {part.casefold() for part in resolved.parts}
    name = resolved.name.casefold()
    parent_name = resolved.parent.name.casefold()

    if any(part in lowered_parts for part in {".ssh", ".gnupg"}):
        return "目标位于 SSH/GPG 私密目录"
    if name in {"id_rsa", "id_dsa", "id_ecdsa", "id_ed25519", "authorized_keys"}:
        return "目标是 SSH 身份或授权文件"
    if resolved.suffix.casefold() in {".pem", ".key", ".p12", ".pfx"}:
        return "目标疑似私钥或证书密钥文件"
    if name == ".env" or (name == "env" and parent_name == "config"):
        return "目标是运行密钥配置；请使用 Settings 保存，不通过模型补丁改写"
...
def _validate_patch_paths(paths: list[str], *, cwd: Path) -> tuple[bool, str]:
    for path in paths:
        token = _normalize_patch_path_token(path)
        if not token or token == "/dev/null":
            continue
        if "\x00" in token:
            return False, "patch path contains a null byte"
        target = _resolve_patch_target(cwd, token)
        safety_reason = _patch_target_safety_reason(target)
        if safety_reason:
            return False, f"安全边界阻止目标 {target}: {safety_reason}。授权工作区和显式普通路径仍可跨目录使用。"
    return True, ""
...
def _raw_content_write_for_target(
    *,
    ...
    if _looks_like_patch(value) or _patch_changed_paths(value):
        return None
    valid, reason = _validate_patch_paths([target], cwd=cwd)
    if not valid:
        return {
            "status": "blocked",
            "tool": "apply_patch",
            ...
            "recovery_hint": [
                "target_file 可以是相对 cwd 的路径，也可以是用户明确要求的绝对路径。",
                "请检查目标路径是否存在、可写，并确认没有把用户指定目录改写成当前 cwd。",
            ],
        }
    target_file = _resolve_patch_target(cwd, target)
...
def _deepseek_structured_preview_patch(cwd: Path, staged: dict[str, str | None]) -> str:
    ...
    for rel_path, new_content in staged.items():
        ...
        if not rel:
            continue
        target = _resolve_patch_target(cwd, rel)
        try:
...
def _execute_deepseek_structured_apply_patch(
    ...
    *,
    ...
    valid, reason = _validate_patch_paths(changed_paths, cwd=cwd)
    if not valid:
        return {
            "status": "blocked",
            "tool": "apply_patch",
            ...
            "recovery_hint": [
                "target_file 可使用相对 cwd 的路径或用户明确指定的绝对路径。",
                "请检查路径语法、目标权限和目标文件状态；apply_patch 会自动创建父目录。",
            ],
        }
    ...
    def read_current(rel_path: str, *, create_if_missing: bool) -> tuple[str | None, str]:
        if rel_path in staged:
            ...
            return current, ""
        target = _resolve_patch_target(cwd, rel_path)
        if target.exists():
    ...
    *,
    ...
    if not valid:
        return {
            ...
            "patch": patch_text,
            "recovery_hint": [
                "structured patch 可使用相对 cwd 的路径或用户明确指定的绝对路径。",
                "请检查路径语法和目标权限后重试。",
            ],
        }
    if not operations:
    ...
    for op in operations:
        path = _resolve_patch_target(cwd, str(op.get("path") or ""))
        ...
        if op_name == "add":
            ...
            target_path = _resolve_patch_target(cwd, str(op.get("move_to") or op.get("path") or ""))
...
def _finalize_apply_patch_result(result: dict[str, Any], *, cwd: Path) -> dict[str, Any]:
    ...
    resolved_paths = [
        str(_resolve_patch_target(cwd, path))
        for path in changed_paths
        if _normalize_patch_path_token(path) not in {"", "/dev/null"}
    ]
    if resolved_paths:
        result["resolved_files"] = resolved_paths
...
def _execute_apply_patch_tool(args: dict[str, Any], context: ToolContext) -> dict[str, Any]:
    ...
    if not raw_valid:
        return {
            ...
            "recovery_hint": [
                "patch header/target_file 可使用相对 cwd 的路径或用户明确指定的绝对路径。",
                "请检查路径语法和目标权限；结构化 operation 更适合绝对路径编辑。",
            ],
        }
...
def _execute_web_search_tool(args: dict[str, Any], context: ToolContext) -> dict[str, Any]:
    ...
    if not api_key:
        ...
        return {
            ...
            "query": query,
            "mode": mode,
            "message": f"未配置 {missing_name}。请到 WebSearch 设置写入 Summary/Web Key。",
        }
...
def _shell_program() -> str:
    def usable(candidate: str, resolved: str) -> bool:
        if os.name == "nt" and Path(candidate).name.lower() in {"bash", "bash.exe", "sh", "sh.exe"}:
    ...
    for candidate in candidates:
        ...
        if os.path.isabs(candidate) and os.path.exists(candidate):
            if usable(candidate, candidate):
                return candidate
            continue
        if not os.path.isabs(candidate):
            found = shutil.which(candidate)
            if found and usable(candidate, found):
                return found
    if os.name == "nt":
...
class ToolRegistry:
        ...
        *,
        ...
        if include_command:
            ...
            self._tools["apply_patch"] = ToolDefinition(
                name="apply_patch",
                description=(
                    "Create or edit files at user-authorized paths. Relative targets resolve from the current "
                    "working directory; absolute targets preserve an explicitly requested destination. DeepSeek should prefer the "
                ...
                input_schema={
                    ...
                    "properties": {
                        ...
                        "file": {
                            "type": "string",
                            "description": "Optional target file, relative to cwd or absolute when explicitly requested. Prefer target_file.",
                        },
                        "path": {
                            "type": "string",
                        ...
                        "target_file": {
                            "type": "string",
                            "description": "Target file path. Relative paths resolve from cwd; absolute paths preserve the requested destination.",
                        },
                        "filename": {
                            "type": "string",
                            "description": "Compatibility alias for file.",
                        ...
                        "edits": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "target_file": {"type": "string", "description": "Target file path, relative to cwd or absolute when the user requested that destination."},
    ...
    def execute_tool_call(self, tool_call: dict[str, Any], context: ToolContext) -> dict[str, Any]:
        ...
        if tool_context.event_callback is not None and tool.name not in {"command", "link", "persona_link", "liaison"}:
            start_payload: dict[str, Any] = {
                "tool": tool.name,
                "brief": str(arguments.get("brief") or "").strip(),
                **_tool_actor_payload(tool_context),
            }
            for key in ("action", "query", "command", "path", "relative_path", "mode"):
                value = arguments.get(key)
                if value is not None and value != "":
                    start_payload[key] = value
            tool_context.event_callback("tool_start", start_payload)
