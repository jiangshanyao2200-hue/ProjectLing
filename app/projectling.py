...
class ProjectLingConfig:
    ...
    deepseek_executor_model: str
    gemini_api_key: str | None
    gemini_base_url: str
    gemini_planner_model: str
    gemini_executor_model: str
    gemini_top_p: float | None
    gemini_top_k: int | None
    gemini_candidate_count: int | None
    gemini_seed: int | None
    gemini_presence_penalty: float | None
    gemini_frequency_penalty: float | None
    gemini_stop_sequences: tuple[str, ...]
...
def deepseek_usage_cache_summary(usage: dict[str, Any] | None) -> dict[str, Any]:
    ...
    hit_tokens = as_int(usage.get("prompt_cache_hit_tokens"))
    if not hit_tokens:
        hit_tokens = as_int(prompt_details.get("cached_tokens"))
    miss_tokens = as_int(usage.get("prompt_cache_miss_tokens"))
    prompt_tokens = as_int(usage.get("prompt_tokens"))
    if not miss_tokens and prompt_tokens and hit_tokens <= prompt_tokens:
    ...
    denominator = cache_total or prompt_tokens
    hit_rate = round((hit_tokens / denominator) * 100, 1) if denominator else 0.0
    return {
        "prompt_tokens": prompt_tokens or cache_total,
...
def _api_provider_value(raw: str | None) -> str:
...
def _normalize_reasoning_effort(raw: str | None) -> str:
    ...
    aliases = {
        ...
        "default": DEEPSEEK_REASONING_EFFORT_DEFAULT,
        "normal": DEEPSEEK_REASONING_EFFORT_DEFAULT,
        "medium": "high",
        "low": "high",
        "xhigh": "max",
        "x-high": "max",
    ...
    if value in {"high", "max"}:
        return value
    return DEEPSEEK_REASONING_EFFORT_DEFAULT


def _normalize_gemini_reasoning_effort(raw: str | None) -> str:
    value = str(raw or "").strip().lower()
    aliases = {
        "": GEMINI_REASONING_EFFORT_DEFAULT,
        "default": GEMINI_REASONING_EFFORT_DEFAULT,
        "off": "none",
        "false": "none",
        "disabled": "none",
        "disable": "none",
    ...
    value = aliases.get(value, value)
    if value in {"none", "low", "high"}:
        return value
    return GEMINI_REASONING_EFFORT_DEFAULT


def _optional_float_value(raw: str | None, *, min_value: float, max_value: float) -> float | None:
...
def _optional_int_value(raw: str | None, *, min_value: int, max_value: int | None = None) -> int | None:
    value = str(raw or "").strip()
    if not value:
        return None
    try:
...
def _card_layout(width: int) -> tuple[str, int]:
    ...
    left_margin = 2 if viewport_width >= 32 else 1 if viewport_width >= 20 else 0
    inner_width = min(78, max(1, viewport_width - left_margin - 1))
...
def is_role_locked(config: ProjectLingConfig | None = None) -> bool:
    config = config or load_config()
    return bool(_read_role_state(config).get("locked"))


def set_role_locked(locked: bool, config: ProjectLingConfig | None = None) -> tuple[LauncherRole, int]:
    config = config or load_config()
    ...
    payload.update(
        {
            ...
            "selected_at": int(payload.get("selected_at") or now),
            "expires_at": int(payload.get("expires_at") or (now + ttl_seconds)),
...
def resolve_active_role(
    ...
    *,
    ...
    runtime_state_read_only = _env_bool(os.environ.get("PROJECTLING_RUNTIME_STATE_READ_ONLY"), default=False)

    if runtime_state_read_only:
        cached_role = _find_role_by_name(roster, cached_name) if cached_name else None
        if cached_role is not None:
            return cached_role, sequence_seed if sequence_seed > 0 else 1
        return roster[0], 1

    if selected_at > 0 and saved_ttl != ttl_seconds and not locked:
        expires_at = selected_at + ttl_seconds

    if cached_name and (locked or expires_at > now):
        cached_role = _find_role_by_name(roster, cached_name)
        if cached_role is not None:
            sequence_seed_missing = sequence_seed <= 0
            if sequence_seed <= 0:
                sequence_seed = random.SystemRandom().randrange(1, 2**31)
            if sequence_seed_missing or saved_ttl != ttl_seconds or int(state.get("expires_at") or 0) != expires_at:
                payload = dict(state)
                payload.update(
                    {
...
def load_config() -> ProjectLingConfig:
    ...
    merged: dict[str, str] = {}
    project_env_layers: list[dict[str, str]] = []
    project_env_paths = {
        config_dir / "env",
    ...
    for candidate in (
        ...
        layer = _load_env_file(candidate)
        merged.update(layer)
        if candidate in project_env_paths:
            project_env_layers.append(layer)
    merged.update({key: value for key, value in os.environ.items() if value is not None})
    for layer in project_env_layers:
        for key in PROJECT_ENV_OVERRIDE_KEYS:
            value = _first_non_empty(layer.get(key))
    ...
    api_provider = _api_provider_value(
        _first_non_empty(
            merged.get("PROJECTLING_API_PROVIDER"),
            merged.get("PROJECTLING_PROVIDER"),
            PROJECTLING_API_PROVIDER_DEFAULT,
        )
    )
    ...
    deepseek_executor_model = _first_non_empty(merged.get("DEEPSEEK_EXECUTOR_MODEL"), DEEPSEEK_FAST_MODEL) or DEEPSEEK_FAST_MODEL
    gemini_api_key = _first_non_empty(merged.get("GEMINI_API_KEY"))
    gemini_base_url = _first_non_empty(merged.get("GEMINI_BASE_URL"), GEMINI_DEFAULT_BASE_URL) or GEMINI_DEFAULT_BASE_URL
    gemini_planner_model = _first_non_empty(merged.get("GEMINI_PLANNER_MODEL"), GEMINI_PRECISE_MODEL) or GEMINI_PRECISE_MODEL
    gemini_executor_model = _first_non_empty(merged.get("GEMINI_EXECUTOR_MODEL"), GEMINI_FAST_MODEL) or GEMINI_FAST_MODEL
    gemini_top_p = _optional_float_value(merged.get("GEMINI_TOP_P"), min_value=0.0, max_value=1.0)
    gemini_top_k = _optional_int_value(merged.get("GEMINI_TOP_K"), min_value=1)
    gemini_candidate_count = _optional_int_value(merged.get("GEMINI_CANDIDATE_COUNT"), min_value=1, max_value=8)
    gemini_seed = _optional_int_value(merged.get("GEMINI_SEED"), min_value=0)
    gemini_presence_penalty = _optional_float_value(merged.get("GEMINI_PRESENCE_PENALTY"), min_value=-2.0, max_value=2.0)
    gemini_frequency_penalty = _optional_float_value(merged.get("GEMINI_FREQUENCY_PENALTY"), min_value=-2.0, max_value=2.0)
    gemini_stop_sequences = _csv_tuple(merged.get("GEMINI_STOP_SEQUENCES"))
    gemini_response_mime_type = str(_first_non_empty(merged.get("GEMINI_RESPONSE_MIME_TYPE"), "") or "").strip()
    gemini_reasoning_effort = _normalize_gemini_reasoning_effort(
    ...
    max_tokens_raw = _first_non_empty(
        ...
        merged.get("DEEPSEEK_MAX_TOKENS"),
        "",
    )
    try:
    ...
    temperature_raw = _first_non_empty(
        merged.get("GEMINI_TEMPERATURE") if api_provider == "gemini" else None,
        merged.get("DEEPSEEK_TEMPERATURE"),
        "0.2",
    )
    try:
    ...
    config = ProjectLingConfig(
        ...
        gemini_planner_model=gemini_planner_model,
        gemini_executor_model=gemini_executor_model,
        gemini_top_p=gemini_top_p,
        gemini_top_k=gemini_top_k,
        gemini_candidate_count=gemini_candidate_count,
        gemini_seed=gemini_seed,
        gemini_presence_penalty=gemini_presence_penalty,
        gemini_frequency_penalty=gemini_frequency_penalty,
        gemini_stop_sequences=gemini_stop_sequences,
...
_FOCUS_SECRET_RE = re.compile(
    r"\b(?:sk-[A-Za-z0-9_-]{10,}|ghp_[A-Za-z0-9_]{10,}|AIza[0-9A-Za-z_-]{10,}|Bearer\s+[A-Za-z0-9._-]{10,})\b",
    flags=re.IGNORECASE,
)
_FOCUS_FILE_RE = re.compile(
    r"(?<![A-Za-z0-9_])(?:[A-Za-z]:[\\/])?(?:[\w .\-\u4e00-\u9fff]+[\\/])*"
    r"[\w .\-\u4e00-\u9fff]+\."
    r"(?:py|cs|html|htm|js|jsx|ts|tsx|css|json|md|ps1|sh|zsh|yml|yaml|toml|xml|csproj|sln|txt|env)"
    r"(?![A-Za-z0-9_])",
    flags=re.IGNORECASE,
)


def _focus_state_path(config: ProjectLingConfig) -> Path:
    return config.runtime_dir / FOCUS_STATE_FILE


def _focus_sanitize_text(value: Any, *, limit: int = 240) -> str:
    text = _FOCUS_SECRET_RE.sub("[secret]", str(value or "")).strip()
    text = re.sub(r"\s+", " ", text)
    return text[:limit].rstrip()


def _focus_unique(values: list[str], *, limit: int = 8) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
    for value in values:
        text = _focus_sanitize_text(value, limit=180)
        if not text:
            continue
...
def _focus_normalize_file_label(value: Any) -> str:
    text = _focus_sanitize_text(value, limit=260).strip("'\"`<>.,:;)]}")
    if not text:
        return ""
    text = text.replace("\\", "/")
    if " " in text and not re.match(r"^[A-Za-z]:/", text):
        ...
        if "/" in tail and "." in tail:
            text = tail
    project_prefix = str(PROJECTLING_DIR).replace("\\", "/").rstrip("/") + "/"
    if text.lower().startswith(project_prefix.lower()):
        text = text[len(project_prefix) :]
    return text.strip("/")


def _focus_extract_files(text: Any) -> list[str]:
    raw = _focus_sanitize_text(text, limit=1200)
    files: list[str] = []
    for match in _FOCUS_FILE_RE.finditer(raw):
        label = _focus_normalize_file_label(match.group(0))
        if label:
            files.append(label)
    return _focus_unique(files, limit=8)


def _focus_iter_payloads(value: Any, *, depth: int = 0) -> Iterator[dict[str, Any]]:
    if depth > 4:
        return
    if isinstance(value, dict):
        yield value
        for item in value.values():
            yield from _focus_iter_payloads(item, depth=depth + 1)
    elif isinstance(value, (list, tuple)):
        for item in value:
            yield from _focus_iter_payloads(item, depth=depth + 1)


def _focus_trace_payloads(trace: dict[str, Any]) -> list[dict[str, Any]]:
    payloads: list[dict[str, Any]] = [trace]
    ...
    if isinstance(arguments, str) and arguments.strip().startswith(("{", "[")):
        ...
        if isinstance(parsed_arguments, dict):
            payloads.append(parsed_arguments)
    result = trace.get("result")
    if isinstance(result, dict):
        payloads.extend(_focus_iter_payloads(result))
    return payloads


def _focus_collect_trace_data(tool_traces: tuple[dict[str, Any], ...]) -> tuple[list[str], list[str], list[str]]:
    files: list[str] = []
    tools: list[str] = []
    summaries: list[str] = []
    text_keys = {
    ...
    for trace in tool_traces:
        if not isinstance(trace, dict):
            continue
        trace_tool = _focus_sanitize_text(trace.get("name"), limit=80)
        if trace_tool:
            tools.append(trace_tool)
        for payload in _focus_trace_payloads(trace):
            tool = _focus_sanitize_text(payload.get("tool") or payload.get("channel"), limit=80)
            if tool:
                tools.append(tool)
            for key, value in payload.items():
                if key in file_keys:
                    label = _focus_normalize_file_label(value)
                    if label:
                        files.append(label)
                if key in text_keys:
                    text = _focus_sanitize_text(value, limit=260)
                    if text:
                        summaries.append(text)
                        files.extend(_focus_extract_files(text))
    return _focus_unique(files, limit=8), _focus_unique(tools, limit=6), _focus_unique(summaries, limit=6)


def _is_context_correction_feedback(user_message: str) -> bool:
...
def _is_short_feedback(user_message: str) -> bool:
    text = re.sub(r"\s+", "", str(user_message or ""))
    if not text:
        return False
    if _is_context_correction_feedback(text):
        return True
    if len(text) > 140:
        return False
    markers = (
...
def _focus_title_from_turn(
    *,
    ...
    combined = " ".join([user_message, *files, *tools, *summaries, str((route or {}).get("category") or "")]).lower()
    if "windows-launcher/program.cs" in combined or ("slash" in combined and "menu" in combined) or "/ menu" in combined:
        return "Windows Launcher · Slash Menu"
    if "settings" in combined or "api" in combined and "model" in combined or "设置" in combined or "模型" in combined or "gemini" in combined:
        return "Settings · Model/API"
    if "receipt" in combined or "tool ui" in combined or "工具回执" in combined or ("显示" in combined and "工具" in combined):
        return "Terminal UI · Tool Timeline"
    if files:
        first = files[0].replace("\\", "/").rsplit("/", 1)[-1]
        if first:
            return f"File Focus · {first}"
    if tools:
        return f"Tool Focus · {tools[0]}"
    category = _focus_sanitize_text((route or {}).get("category"), limit=80)
    return f"Current Task · {category}" if category else "Current Task"


def _derive_focus_from_turn(
    *,
    user_message: str,
    assistant_text: str,
    tool_traces: tuple[dict[str, Any], ...],
    ...
    if not tool_traces and _is_short_feedback(user_message):
        return None
    files, tools, summaries = _focus_collect_trace_data(tool_traces)
    user_summary = _focus_sanitize_text(user_message, limit=180)
    assistant_summary = _focus_sanitize_text(assistant_text, limit=180)
    files = _focus_unique([*files, *_focus_extract_files(user_summary), *_focus_extract_files(assistant_summary)], limit=8)
    summary_parts = _focus_unique([user_summary, *summaries, assistant_summary], limit=4)
    category = str((route or {}).get("category") or "").strip()
    meaningful = bool(tool_traces or files or category not in {"", "casual_chat"} or len(user_summary) >= 12)
    if not meaningful:
        return None
    title = _focus_title_from_turn(
    ...
    target = files[0] if files else (summary_parts[0] if summary_parts else title)
    state: dict[str, Any] = {
        "schema": 1,
        "title": title,
        "target": target,
        "summary": " / ".join(summary_parts)[:360],
        "files": files,
        "tools": tools,
        "route": category,
    ...
    if role is not None:
        state["role"] = f"{role.name_zh} / {role.name_en}"
    return state


def _load_focus_state(config: ProjectLingConfig) -> dict[str, Any]:
    state = _load_json_file(_focus_state_path(config))
    if not isinstance(state, dict):
        return {}
    title = _focus_sanitize_text(state.get("title"), limit=120)
    if not title:
        return {}
    return {
        "schema": 1,
        "title": title,
        "target": _focus_sanitize_text(state.get("target"), limit=180),
        "summary": _focus_sanitize_text(state.get("summary"), limit=360),
        "files": _focus_unique([str(item) for item in state.get("files") or []], limit=8),
        "tools": _focus_unique([str(item) for item in state.get("tools") or []], limit=6),
        "route": _focus_sanitize_text(state.get("route"), limit=80),
        "role": _focus_sanitize_text(state.get("role"), limit=120),
        "updated_at": _focus_sanitize_text(state.get("updated_at"), limit=80),
    }


def _save_focus_state(config: ProjectLingConfig, state: dict[str, Any]) -> None:
    ...
    cleaned.update(
        {
            ...
            "route": _focus_sanitize_text(state.get("route"), limit=80),
            "role": _focus_sanitize_text(state.get("role"), limit=120),
            "updated_at": _focus_sanitize_text(state.get("updated_at") or time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), limit=80),
        }
    )
    if not cleaned["title"]:
        return
    _write_json_file(_focus_state_path(config), cleaned)


def _focus_system_message(config: ProjectLingConfig, user_message: str) -> dict[str, Any] | None:
    state = _load_focus_state(config)
    if not state:
        return None
    title = state.get("title") or "Current Task"
    target = state.get("target") or "-"
    summary = state.get("summary") or "-"
    files = ", ".join(state.get("files") or []) or "-"
    tools = ", ".join(state.get("tools") or []) or "-"
    short_feedback = _is_short_feedback(user_message)
    context_correction = _is_context_correction_feedback(user_message)
    lines = [
        "当前焦点锚点（隐藏给用户，不要原样输出）：",
        f"- FOCUS: {title}",
        f"- target: {target}",
    ...
    if short_feedback:
        lines.append("- 本轮用户很可能在反馈刚才的焦点工作；优先按当前焦点理解用户反馈，不要跳到无关文件/任务。")
    if context_correction:
        lines.append("- 如果用户指出丢上下文或答非所问，先做 Context Check，回到当前焦点，再继续处理。")
    else:
        lines.append("- 若用户新请求明显不属于该焦点，再切换目标；不确定时先用一句话确认当前指向。")
...
class DeepSeekClient:
    ...
    @staticmethod
    def _supports_thinking_control(configured_model: str) -> bool:
        model = str(configured_model or "").strip().lower()
        return model.startswith("deepseek-v4-") or model in {"deepseek-chat", "deepseek-reasoner"}

    def _supports_gemini_reasoning_effort(self, configured_model: str) -> bool:
        model = str(configured_model or "").strip().lower()
        return self.provider() == "gemini" and ("gemini" in model)

    def _supports_gemini_zero_thinking_budget(self, configured_model: str) -> bool:
        model = str(configured_model or "").strip().lower()
        return self.provider() == "gemini" and "flash" in model and (
    ...
    def _deep_merge_dict(target: dict[str, Any], source: dict[str, Any]) -> None:
        for key, value in source.items():
            ...
            if isinstance(value, dict) and isinstance(existing, dict):
                DeepSeekClient._deep_merge_dict(existing, value)
            else:
                target[key] = value

    def _merge_extra_body(self, payload: dict[str, Any], extra_body: dict[str, Any]) -> None:
        if not extra_body:
            return
    ...
    @staticmethod
    def _has_gemini_thinking_config(extra_body: dict[str, Any]) -> bool:
        google = extra_body.get("google") if isinstance(extra_body.get("google"), dict) else {}
        return isinstance(google, dict) and (
            isinstance(google.get("thinking_config"), dict) or isinstance(google.get("thinkingConfig"), dict)
        )

    @classmethod
    def _sanitize_gemini_schema(cls, value: Any) -> Any:
        if isinstance(value, list):
            return [cls._sanitize_gemini_schema(item) for item in value]
        if not isinstance(value, dict):
            return value
        cleaned: dict[str, Any] = {}
        for key, item in value.items():
            if key in {"anyOf", "oneOf", "allOf", "$schema", "$defs", "definitions"}:
                continue
            if key == "additionalProperties" and item is False:
                continue
            cleaned[key] = cls._sanitize_gemini_schema(item)
        schema_type = str(cleaned.get("type") or "").strip().lower()
        if "required" in cleaned:
            if schema_type and schema_type != "object":
                cleaned.pop("required", None)
            else:
                props = cleaned.get("properties") if isinstance(cleaned.get("properties"), dict) else {}
                cleaned["required"] = [name for name in cleaned.get("required") or [] if isinstance(name, str) and name in props]
                if not cleaned["required"]:
                    cleaned.pop("required", None)
        return cleaned

    @classmethod
    def _sanitize_gemini_tools(cls, tools: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
        if not tools:
            return tools
        ...
        for tool in tools:
            if not isinstance(tool, dict):
                continue
            entry = json.loads(json.dumps(tool, ensure_ascii=False))
            fn = entry.get("function") if isinstance(entry.get("function"), dict) else {}
            if isinstance(fn.get("parameters"), dict):
                fn["parameters"] = cls._sanitize_gemini_schema(fn["parameters"])
    ...
    @classmethod
    def _audit_numeric_usage(cls, value: Any, *, depth: int = 0) -> dict[str, Any]:
        ...
        for raw_key, raw_value in list(value.items())[:64]:
            ...
            if isinstance(raw_value, bool):
                ...
                nested = cls._audit_numeric_usage(raw_value, depth=depth + 1)
                if nested:
                    cleaned[key] = nested
        return cleaned

    def _payload_thinking_enabled(self, payload: dict[str, Any]) -> bool:
        ...
        google = extra_body.get("google") if isinstance(extra_body.get("google"), dict) else {}
        thinking_config = google.get("thinking_config") if isinstance(google.get("thinking_config"), dict) else {}
        if not thinking_config and isinstance(google.get("thinkingConfig"), dict):
            thinking_config = google.get("thinkingConfig") or {}
        if thinking_config:
            budget = thinking_config.get("thinking_budget", thinking_config.get("thinkingBudget"))
            if isinstance(budget, (int, float)):
                return budget != 0
            return True
        return self._thinking_enabled_for_request(configured_model=str(payload.get("model") or ""))

    @staticmethod
    def _audit_response_header(response: Any, *names: str) -> str:
    ...
    def _write_model_request_audit(
        self,
        ...
        *,
        ...
        configured_planner, configured_executor, planner_model, executor_model = self._configured_model_pair()
        usage = self._audit_numeric_usage((response_data or {}).get("usage"))
        choices = (response_data or {}).get("choices") or []
        first_choice = choices[0] if isinstance(choices, list) and choices and isinstance(choices[0], dict) else {}
        response_model = str((response_data or {}).get("model") or "").strip()[:160]
        response_id = str((response_data or {}).get("id") or "").strip()[:200]
        try:
            http_status = int(getattr(response, "status", None) or getattr(response, "code", None) or 0)
        except (TypeError, ValueError):
            http_status = 0
        record: dict[str, Any] = {
            ...
            "latency_ms": int(max(0.0, time.monotonic() - started_at) * 1000),
            "attempts": max(1, int(attempts or 1)),
            "status": str(status or "unknown")[:40],
            "http_status": http_status or None,
            "request_id": self._audit_response_header(response, "x-request-id", "x-trace-id", "request-id"),
            "response_id": response_id,
            "finish_reason": str(first_choice.get("finish_reason") or "").strip()[:80],
            "usage": usage,
            "error_type": type(error_value).__name__ if error_value is not None else "",
        }
        audit_path = path
        if audit_path is None:
            override = str(os.environ.get("PROJECTLING_MODEL_AUDIT_PATH") or "").strip()
            if override.lower() in {"0", "off", "false", "disabled"}:
                return record
            audit_path = Path(override).expanduser() if override else MODEL_REQUEST_AUDIT_LOG
        try:
            audit_path.parent.mkdir(parents=True, exist_ok=True)
            line = json.dumps(record, ensure_ascii=False, separators=(",", ":")) + "\n"
            with _MODEL_REQUEST_AUDIT_LOCK:
        ...
        *,
        ...
        if self.provider() == "gemini":
            ...
            if (
                ...
                and self._supports_gemini_zero_thinking_budget(configured_model)
                and not self._has_gemini_thinking_config(gemini_extra_body)
            ):
                google = gemini_extra_body.setdefault("google", {})
                if isinstance(google, dict):
                    google["thinking_config"] = {"thinking_budget": 0}
            self._merge_extra_body(payload, gemini_extra_body)
            extra_body = payload.get("extra_body") if isinstance(payload.get("extra_body"), dict) else {}
    ...
    def _build_get_request(self, path: str) -> request.Request:
    ...
    def list_models(self) -> dict[str, Any]:
        if not self._api_key():
            raise DeepSeekAPIError(self._api_key_missing_message())
        body = ""
        try:
                    ...
                    with request.urlopen(req, timeout=self._request_timeout(stream=True)) as response:
                        ...
                        for raw_payload in self._iter_sse_payloads(response):
                            ...
                            if first_choice.get("finish_reason"):
                                audit_response["choices"] = [{"finish_reason": first_choice.get("finish_reason")}]
                            yielded_any = True
                            yield chunk
                    self._write_model_request_audit(
                        payload,
...
class ProjectLingEngine:
    ...
    def _fresh_project_bootstrap_plan(user_message: str, *, cwd: Path, context_percent: int) -> dict[str, Any]:
        ...
        if is_android:
            goal = "在用户指定目标路径完成一个可编译安装的 Android APK 项目；未指定时使用当前 cwd。"
            steps = [
                "用 update_plan 建立 APK 创建、编译、安装、验证步骤",
                "检查本机 Android 构建工具和 adb 可用性",
                "使用 apply_patch.operation=write 写入用户指定目标或当前 cwd，并让工具自动创建父目录",
                "运行构建脚本产出 APK，并按工具事实修正编译错误",
                "使用 adb 安装并做最小启动/包名验证",
            ]
            executor_brief = (
                "在用户指定目标路径创建记事本 APK 项目；用户未指定时使用当前 cwd。"
                "target_file 可用相对 cwd 路径或用户明确给出的绝对路径，不要擅自改写目的地。"
                "所有文件创建/修改都用 apply_patch.operation=write/replace，"
                "apply_patch 会自动创建父目录；命令只用于检查环境、编译、adb 安装和验证。"
                "Android build.sh 的 debug keystore 应放在不会被清空的路径，避免每次重编译都签名变化导致 install -r 失败。"
            )
            risks = ["保留用户指定目的地", "不要用 shell 重定向/tee/cat 回退写文件", "编译失败后先读错误再最小修改"]
        elif is_web:
            goal = "在用户指定目标路径创建可直接运行的网页/脚本产物；未指定时使用当前 cwd。"
            steps = [
                "用 update_plan 建立创建、验证、收尾步骤",
                "使用 apply_patch.operation=write 写入用户指定文件；未指定文件名时使用 index.html",
                "运行最小语法/结构检查",
                "报告文件路径和打开方式",
            ]
            executor_brief = (
                "在用户指定目标路径创建网页，未指定时在当前 cwd 创建 index.html；"
                "target_file 可用相对 cwd 路径或用户明确给出的绝对路径，"
                "用 apply_patch.operation=write 落盘，不要把完整源码直接吐给用户。"
            )
            risks = ["不要把用户指定目录静默改成 cwd", "不要用 command 写文件回退"]
        else:
            goal = "在用户指定目标路径创建项目/文件产物；未指定时使用当前 cwd。"
            steps = [
                "用 update_plan 建立真实任务步骤",
                "检查必要环境和已有文件",
                "使用 apply_patch 在用户指定目标或当前 cwd 创建/修改文件",
                "运行最小验证并修正明显错误",
                "报告产物路径和验证结果",
            ]
            executor_brief = (
                "按用户任务保留目标路径；未指定目标时使用当前 cwd。"
                "target_file 可用相对 cwd 路径或用户明确给出的绝对路径，"
                "文件编辑使用 apply_patch.operation=write/replace，命令只做检查和验证。"
            )
            risks = ["不要擅自改写用户目的地", "不要把源码作为正文替代落盘", "工具失败后先缩小改动重试"]
        return {
            "goal": goal,
            "plan": steps,
            "risks": risks,
    ...
    def _run_planner_step(
        ...
        *,
        ...
        planner_model = self._planner_model_for_mode(mode)
        planner_thinking = self._planner_thinking_for_mode(mode)
        ...
        if file_creation_like and not cwd_has_files and bool(route.get("plan_required")):
            ...
            prompt = _prompt_block(
                ...
                文件创建约定：
                - 如果用户明确指定 Desktop、另一个项目目录或绝对路径，必须保留该目的地；未指定时才默认当前 cwd。
                - apply_patch target_file 可使用相对 cwd 的路径或用户明确指定的绝对路径；不要把外部目标静默重定向进当前 cwd。
                ...
                planner_text = json.dumps(
                    {
                        ...
                        "risks": [str(exc)],
                        "context_percent": default_context_percent,
                        "executor_brief": "Planner 不可用，按用户任务稳妥推进；文件编辑必须使用 apply_patch。相对路径以当前 cwd 为基准，用户明确指定的绝对路径和其他授权工作区必须保留，不得静默改写目的地。",
                    },
                    ensure_ascii=False,
                )
                reasoning_text = ""
        ...
        return {
            ...
            "context_budget_text": f"CTK{context_percent}%",
        ...
        *,
        ...
        if visible_thought:
            thinking_trace = {
                ...
                "actor_label": "主星",
                "actor_name": f"{role.name_zh} / {role.name_en}",
                "context_budget_percent": payload.get("context_percent"),
            }
    ...
    def _planner_visible_thought_text(payload: dict[str, Any]) -> str:
        ...
        if reasoning_text:
            return ProjectLingEngine._strip_context_percent_marker(reasoning_text)
        parts: list[str] = []
        ...
        if context_percent not in {None, ""}:
            try:
                parts.append(f"CTK next {max(0, min(100, int(context_percent)))}%")
            except (TypeError, ValueError):
                parts.append(f"CTK next {context_percent}")
        ...
        *,
        ...
        if visible_text:
            thinking_trace = {
                ...
                "actor_label": "主星",
                "actor_name": f"{role.name_zh} / {role.name_en}",
                "context_budget_percent": load_context_budget(self.config).get("percent"),
            }
            thinking_traces.append(thinking_trace)
            if on_stream_event is not None:
                on_stream_event("thinking_trace", thinking_trace)
    ...
    def _select_request_route(
        ...
        *,
        ...
        configured_model = self.config.model.strip() or DEEPSEEK_FAST_MODEL
        collab_mode = _collab_mode_value(getattr(self.config, "collab_mode", PROJECTLING_COLLAB_MODE_DEFAULT))
        planner_model = self._planner_model_for_mode(collab_mode)
        executor_model = self._executor_model_for_mode(collab_mode)
        planner_thinking = self._planner_thinking_for_mode(collab_mode)
        executor_thinking = self._executor_thinking_for_mode(collab_mode)
        ...
        return {
            ...
            "show_initial_status": bool(
                request_thinking_enabled or route_category not in {"casual_chat", "strict_short_reply"}
            ),
    ...
    def _collaboration_mode_prompt(self) -> str:
        ...
        executor_model = self._executor_model_for_mode(mode)
        mode_summary = {
            "rapid": "flash no-think + flash no-think",
            "standard": "flash think + flash no-think",
            "precise": "pro think + pro think",
    ...
    def _tool_schemas_for_request(self, *, scope: str = "full") -> list[dict[str, Any]]:
        ...
        if scope == "plan_command":
            allowed = {"link", "update_plan", "command"}
            schemas = [
                item
                for item in schemas
                if str((item.get("function") or {}).get("name") or "") in allowed
            ...
            return sorted(
                schemas,
                key=lambda item: (
                    {"update_plan": 0, "command": 1, "link": 2}.get(
                        str((item.get("function") or {}).get("name") or ""),
                        10,
                    ),
                    str((item.get("function") or {}).get("name") or ""),
    ...
    def _save_next_context_budget_from_model(
        ...
        *,
        ...
        cleaned_reasoning = self._strip_context_percent_marker(reasoning_text)
        if cleaned_reasoning != reasoning_text:
            assistant_message["reasoning_content"] = cleaned_reasoning
    ...
    def _build_messages(
        ...
        *,
        ...
        if not strict_short_reply and not casual_chat:
            focus_message = _focus_system_message(self.config, user_message)
    ...
    @staticmethod
    def _validate_tool_call_message_order(messages: list[dict[str, Any]]) -> tuple[bool, str]:
        index = 0
        while index < len(messages):
            ...
            if not isinstance(tool_calls, list) or not tool_calls:
                index += 1
                continue
            expected_ids = [str(call.get("id") or "").strip() for call in tool_calls if isinstance(call, dict)]
            expected_ids = [call_id for call_id in expected_ids if call_id]
            if len(expected_ids) != len(tool_calls):
                return False, f"assistant tool_calls at index {index} contain empty ids"
            following = messages[index + 1 : index + 1 + len(expected_ids)]
            if len(following) != len(expected_ids):
                return False, f"assistant tool_calls at index {index} missing tool responses"
            for expected_id, tool_message in zip(expected_ids, following):
                if not isinstance(tool_message, dict) or str(tool_message.get("role") or "") != "tool":
                    return False, f"assistant tool_calls at index {index} not followed by contiguous tool messages"
                if str(tool_message.get("tool_call_id") or "").strip() != expected_id:
                    return False, f"tool_call_id mismatch after assistant index {index}"
    ...
    def _finalize_chat_result(
        ...
        *,
        ...
        focus_state = _derive_focus_from_turn(
            user_message=user_message,
            assistant_text=result.text,
            tool_traces=result.tool_traces,
            route=result.routing or {},
            role=role,
        )
        if focus_state is not None:
            _save_focus_state(self.config, focus_state)
    ...
    def chat(
        ...
        *,
        ...
        while True:
            ...
            order_ok, order_error = self._validate_tool_call_message_order(messages)
            if not order_ok:
            ...
            cache_usage = deepseek_usage_cache_summary(response.get("usage") if isinstance(response, dict) else None)
            if cache_usage:
                response["_projectling_cache_usage"] = cache_usage
                route["cache_usage"] = cache_usage
            ...
            visible_reasoning_text = self._strip_context_percent_marker(reasoning_text)
            if visible_reasoning_text:
                thinking_traces.append(
                    {
                        ...
                        "role": "executor" if execution_role is not None else "planner",
                        "actor_label": "执行星" if execution_role is not None else "主星",
                        "actor_name": f"{model_role.name_zh} / {model_role.name_en}",
                        "context_budget_percent": context_budget.get("percent"),
                    }
                )
            ...
            if has_tool_calls and tools_enabled:
                ...
                parsed_tool_results: list[dict[str, Any]] = []
                for tool_call in tool_calls:
                    ...
                    if on_stream_event is not None:
                        on_stream_event("tool_result", parsed_result)
                    conversation_messages.append(tool_result)
                    parsed_tool_results.append(parsed_result)
                for parsed_result in parsed_tool_results:
